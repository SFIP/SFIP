      ...
      while(1) {
        ...
        // check if the change in status of the child was due to seccomp
          ...
          if(syscalls_allowed[syscall_number] != 1) {
            char question[256];
            #ifdef __x86_64__
              sprintf(question, "%s --yesno 'Process (%d) wants to perform syscall %s (%d). Do you want to allow it?'", (getenv("TOOL")) ? getenv("TOOL") : "true", changed_pid, syscall_list[syscall_number], syscall_number);
            #else
            ...
            if(question_result != 0) {
              debug("User denied requested syscall. Application might currently be exploited, terminating it.\n");
              kill(child, SIGKILL);
